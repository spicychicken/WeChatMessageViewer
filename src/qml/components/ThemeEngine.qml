...
Item {
    ...
    property bool isHdpi: (pScreen.screenDpi >= 128 || pScreen.screenPar >= 2.0)
    ...
    property int componentHeight: isHdpi ? 34 : 38
    property int componentHeightL: isHdpi ? 42 : 46
    property int componentHeightXL: isHdpi ? 50 : 54
    ...
    Component.onCompleted: loadTheme("THEME_DESKTOP_LIGHT")
    ...
    function loadTheme(newIndex) {
        ...
        if (themeIndex < 0 || themeIndex >= ThemeEngine.THEME_LAST) {
            themeIndex = ThemeEngine.THEME_LIGHT_AND_WARM
        }

...
Node* Array::at(unsigned int array_index)
{
    return _array.at(array_index);
}
...
void Array::Set(unsigned int index, const Node* node)
{
    if (node)
    {
        Node* clone = node->Clone();
        UpdateNodeParent(clone);
        plist_array_remove_item(_node, index);
        plist_array_insert_item(_node, clone->GetPlist(), index);
    }
}
